...
S      = 50
BASE_SEED = 42
N_OUT  = 400        # fixed reconstruction-grid size for all runs
...
def _fit_tanh(x, u, A_hint, xc_hint, nu_hint, nu_lo=0.05, nu_hi=2.0,
              strict=False, method_out=None):
    """
    Fit u = -A_f * tanh(A_f*(x-xc_f)/(2*nu_f)) with SciPy nonlinear least
    squares.  Returns (xc_fit, nu_fit, A_fit).

    ``nu_lo``/``nu_hi`` bound the fitted effective viscosity. The defaults
    (0.05, 2.0) reproduce the production and ablation configuration exactly.
    The multi-viscosity sweep lowers ``nu_lo`` below the smallest prescribed
    viscosity so that a sharp layer is not clipped by the fit bound.

    ``strict=True`` re-raises any curve_fit failure instead of falling back to
    the cruder zero-crossing estimate; manuscript study drivers pass it so a
    silent fallback can never enter a reported value. ``method_out``, if a
    dict, receives ``{'method': 'curve_fit' | 'fallback'}`` so per-run records
    can archive which estimator produced each fitted value.
    """
    try:
        ...
        bounds = ([0.5, 0.0, nu_lo], [2.0, float(L), nu_hi])
        popt, _ = curve_fit(model, x, u, p0=p0, bounds=bounds, maxfev=5000)
        if method_out is not None:
            method_out['method'] = 'curve_fit'
        return float(popt[1]), float(popt[2]), float(popt[0])
    except Exception as exc:
        if strict:
            raise RuntimeError(
                f"tanh curve_fit failed and strict fitting is required: {exc}"
            ) from exc
    if method_out is not None:
        method_out['method'] = 'fallback'
...
def _run_one(N, seed, a_rel=A_REL, dt=DT, collect_label=False):
    """Run one simulation through the standalone Paper 2 solver entry point.

    Returns dict of per-run quantities, or None on unrecoverable failure.
    ``dt`` defaults to the paper time step so existing callers are unchanged.
    ``collect_label`` enables the read-only label-variance diagnostic, which
    does not alter the solver's random draws or outputs.
    """
    ...
    diag = {} if collect_label else None
    ...
    # Secondary: fit tanh for center and width. Manuscript runs are strict: a
    # curve_fit failure aborts the run instead of silently falling back.
    fit_diag = {}
    ...
    return {
        ...
        'nu_fit':     nu_fit,
        'A_fit':      A_fit,
        'fit_method': fit_diag.get('method', ''),
        'max_abs_u':  max_abs_u,
        'total_mass_expected': total_mass_expected,
        'runtime_s':  elapsed,
        'u_out':      u_out,   # kept in memory for decomposition
        'x_out':      x_out,
        'label_excess_mean': (diag or {}).get('label_excess_mean'),
        'D_label':    (diag or {}).get('D_label'),
        'rho_label':  (diag or {}).get('rho_label'),
    }
...
def run_study():
    ...
    print(f"#  solver: simulate_burgers_relaxation_gbmc()")
        ...
        for s_idx, seed in enumerate(seeds):
            ...
            if r is None or r.get('failed'):
                ...
                rec_rows.append({
                    ...
                    'l1': '', 'l2': '', 'linf': '', 'rel_l2': '',
                    'xc_fit': '', 'nu_fit': '', 'A_fit': '', 'fit_method': '',
    ...
    fieldnames  = ['N', 'seed', 'failed', 'reason', 'l1', 'l2', 'linf',
                   'rel_l2', 'xc_fit', 'nu_fit', 'A_fit', 'fit_method',
                   'max_abs_u', 'total_mass_expected', 'outside_N',
                   'outside_signed', 'outside_abs', 'runtime_s']
    with open(per_run_csv, 'w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=fieldnames, extrasaction='ignore')
        w.writeheader(); w.writerows(all_per_run)

    # Complete realization-level archive: every seed profile at every N, plus
    # the seed order, so the bias/spread/total table and the realization-level
    # spread-slope bootstrap interval can be recomputed from the archive alone.
    _savez_deterministic(
        os.path.join(OUT_BASE, 'production_profiles_full.npz'),
        seeds=np.asarray(seeds, dtype=np.int64),
        x=x_ref, u_exact=u_ref,
        **{f'N{N}': all_profiles_by_N[N] for N in N_SEQ
           if N in all_profiles_by_N},
    )
    ...
    meta = {
        'git_commit': git_hash,
        'entry_point': 'simulate_burgers_relaxation_gbmc() in relaxation_gbmc.py',
